...
from .utils.utils import filter_range, filter_noise


class Spectrum(MsData):
    """Spectrum object for accessing and handling MS spectral data."""
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(element, obo_version=obo_version)
        ...
        self._raw_peaks: NDArray[np.float64] | None = None
        self._peak_dict: dict[str, NDArray[np.float64] | None] = {
            PeakType.PROFILE: None,
        ...
        logger.debug(f"Spectrum initialized with ID: {self.ID}")

    def __del__(self) -> None:
        """Clear element to reduce RAM usage."""
    ...
    def __getitem__(self, accession: str) -> str | float | bool | list[str | float] | None:
        ...
        if not accession.startswith("MS:"):
            try:
                ...
                accession = obo_entry["id"] if obo_entry is not None else "---"
    ...
    def TIC(self) -> float:
        ...
        if self._tic is None and self.element is not None:
    ...
    def id_dict(self) -> dict[str, str | int]:
        ...
        if self._id_dict is None:
            ...
            spec_id = (
                self.element.attrib.get("id", "") if self.element is not None else ""
            )
    ...
    def ms_level(self) -> int | None:
        ...
        if self._ms_level is None and self.element is not None:
    ...
    def scan_time(self) -> tuple[float | None, str | None]:
        ...
        if (self._scan_time is None or self._scan_time_unit is None) and self.element is not None:
    ...
    def transform_scan_time(self, time_unit: TimeUnit = TimeUnit.SECOND) -> float:
    ...
    def peaks(self, 
              peak_type: str | PeakType = PeakType.PROFILE,
        ...
        if self._peak_dict[peak_type] is None:
            ...
            if peak_type == PeakType.PROFILE:
                raise NotImplementedError("Spectra did not have profile data.")
    ...
    def get_all_arrays_in_spec(self, not_found_array: str | None = None) -> list[str]:
        ...
        b_data_arrays = (
            self.element.findall(b_data_string) if self.element is not None else []
        )
    ...
    def _centroid_peaks(self) -> NDArray[np.float64]:
        ...
        if self.is_profile is None:
            raise RuntimeError("Cannot determine if spectrum is profile or centroided.")
        if self.is_profile:
            return self._centroid_peaks_numpy()
        else:
            if self._raw_peaks is None:
                raise RuntimeError("Raw peaks not yet set, should never happen here.")
            return self._raw_peaks
    ...
    def _centroid_peaks_numpy(self) -> NDArray[np.float64]:
        ...
        if self._peak_dict["reprofiled"] is not None:
            ...
            raw_peaks = self.peaks(PeakType.PROFILE)
            i_array = np.asarray(raw_peaks[:, 1], dtype=np.float64)
            mz_array = np.asarray(raw_peaks[:, 0], dtype=np.float64)
    ...
    def find_peaks(self, mz: float, tolerance: float, ppm_tol: bool = True, peak_type: PeakType = PeakType.CENTROIDED) -> list[tuple[float, float]]:
        ...
        matched_peaks = [(mz_val, intensity) for mz_val, intensity in peaks if mz_min <= mz_val <= mz_max]
        if matched_peaks:
            return matched_peaks
        return []
    
    def has_peak(self, mz: float, tolerance: float, ppm_tol: bool = True, peak_type: PeakType = PeakType.CENTROIDED) -> list[tuple[float, float]] | None:
        """Check if spectrum has peak at given m/z within precision tolerance."""
    ...
    def has_overlapping_peak(self, mz: float, tolerance: float, ppm_tol: bool = True, peak_type: PeakType = PeakType.CENTROIDED) -> bool:
        """Check if spectrum has multiple peaks near given m/z."""
        matched_peaks = self.find_peaks(mz, tolerance, ppm_tol, peak_type)
        return len(matched_peaks) > 1
        

    def similarity_to(self, spec2: "Spectrum", round_precision: int = 0, peak_type: PeakType = PeakType.CENTROIDED) -> float:
    ...
    def ppeaks(self) -> NDArray[np.float64]:
        return self.peaks(PeakType.PROFILE) 
    
    @property
    def dpeaks(self) -> NDArray[np.float64]:
        return self.peaks(PeakType.DECONVOLUTED)
